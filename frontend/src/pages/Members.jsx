...
import { Search, Plus, X, QrCode, RefreshCw, ChevronRight, User, Download, MessageCircle } from 'lucide-react'
import { QRCodeSVG } from 'qrcode.react'
import { generateReceipt, exportMembersPDF, whatsappLink } from '../utils/pdf'
...
function Badge({ status, daysLeft }) {
  ...
  return <span className="text-[10px] font-bold px-2 py-1 rounded-full" style={{ background: c.bg, color: c.color, border: `1px solid ${c.border}` }}>{c.label}</span>
}
    ...
    <div className="grid grid-cols-3 gap-2">
      {[['cash', '💵 Cash'], ['flooz', '📱 Flooz'], ['tmoney', '📱 T-Money']].map(([m, lbl]) => (
        <button key={m} type="button" onClick={() => onChange(m)}
          ...
          style={{ background: value === m ? 'linear-gradient(135deg, #F59E0B, #D97706)' : 'rgba(15,23,42,0.8)', color: value === m ? '#0F172A' : '#94A3B8', border: value === m ? 'none' : '1px solid rgba(255,255,255,0.08)' }}>
          {lbl}
        </button>
      ))}
    </div>
...
function Modal({ title, onClose, children }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }}>
      <div className="w-full max-w-md rounded-2xl overflow-hidden max-h-[90vh] overflow-y-auto" style={{ background: '#0D1525', border: '1px solid rgba(255,255,255,0.08)' }}>
        <div className="flex items-center justify-between px-6 py-4 sticky top-0" style={{ background: '#0D1525', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
...
function AddMemberModal({ plans, onClose, onSave, gym }) {
  ...
  const addSub = async () => {
    ...
    setLoading(true)
    try {
      const res = await subscriptionsAPI.create({ memberId: newMember.id, ...subForm })
      setNewSub(res.data)
      // Générer reçu PDF
      const plan = plans.find(p => p.id === subForm.planId)
      generateReceipt({
        member: newMember,
        subscription: { ...res.data, plan },
        gym
      })
      onSave()
    }
  ...
  const inp = "w-full text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none transition-all"
  const inpStyle = { background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(255,255,255,0.08)' }

  return (
    <Modal title={step === 1 ? 'Nouveau membre' : 'Abonnement'} onClose={onClose}>
      {step === 1 ? (
        <div className="space-y-4">
          {[['Nom complet *', 'fullName'], ['Téléphone', 'phone'], ['Email', 'email']].map(([label, key]) => (
            <div key={key}>
              <label className="block text-xs text-[#64748B] mb-1.5 font-semibold uppercase tracking-wider">{label}</label>
              <input value={form[key]} onChange={e => setForm({...form, [key]: e.target.value})} className={inp} style={inpStyle}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select value={subForm.planId} onChange={e => setSubForm({...subForm, planId: e.target.value})}
              className="w-full text-white rounded-xl px-4 py-2.5 text-sm" style={inpStyle}>
          ...
          <button onClick={addSub} disabled={loading}
            className="w-full py-3 rounded-xl font-bold text-sm text-[#0F172A] disabled:opacity-40"
            style={{ background: 'linear-gradient(135deg, #F59E0B, #D97706)' }}>
            {loading ? '...' : subForm.planId ? '✓ Confirmer + Générer reçu PDF' : 'Terminer sans abonnement'}
          </button>
        </div>
      )}
    </Modal>
...
function MemberModal({ member, plans, onClose, onSave, gym }) {
  ...
  const handleRenew = async () => {
    ...
    setLoading(true)
    try {
      const res = await subscriptionsAPI.create({ memberId: member.id, planId, paymentMethod: payMethod })
      const plan = plans.find(p => p.id === planId)
      generateReceipt({ member: { fullName: member.full_name, phone: member.phone }, subscription: { ...res.data, plan }, gym })
      onSave()
    }
    catch (e) { alert(e.response?.data?.error || 'Erreur') }
    finally { setLoading(false) }
  ...
  const hasPhone = member.phone && member.phone.length >= 8
  const hasEndDate = member.currentSubscription?.end_date
    ...
    <Modal title={member.full_name} onClose={onClose}>
      <div className="space-y-4">
        <div className="flex items-center gap-4 p-4 rounded-xl" style={{ background: 'rgba(15,23,42,0.6)' }}>
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-lg font-black" style={{ background: 'linear-gradient(135deg, #F59E0B, #D97706)', color: '#0F172A' }}>
        ...
        {member.currentSubscription && (
          <div className="p-4 rounded-xl" style={{ background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <p className="text-[#64748B] text-xs mt-1">
              ...
              {member.daysLeft > 0 ? ` · ${member.daysLeft} jours restants` : ''}
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          {hasPhone && hasEndDate && (
            <a href={whatsappLink(member.phone, member.full_name, gym?.name, member.currentSubscription?.end_date)}
...
export default function Members() {
  ...
  const handleExport = () => exportMembersPDF({ members, gym })
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#475569] text-xs mt-1">{members.length} membres</p>
        </div>
      ...
      {showAdd && <AddMemberModal plans={plans} gym={gym} onClose={() => setShowAdd(false)} onSave={() => { setShowAdd(false); load() }} />}
      {selected && <MemberModal member={selected} plans={plans} gym={gym} onClose={() => setSelected(null)} onSave={() => { setSelected(null); load() }} />}
    </div>
  )
}
